...
# ══════════════════════════════════════════════════════════════════════════════
# ══════════════════════════════════════════════════════════════════════════════
# SHARED: CODEBASE EXPOSURE HELPERS
# ══════════════════════════════════════════════════════════════════════════════
_SUPPORTED_EXTS = {
...
def _load_zip_codebase(zip_bytes: bytes) -> dict:
    contents = {}
    ...
    try:
        with zipfile.ZipFile(tmp_path, "r") as z:
            for name in z.namelist():
                ...
                if ext in _SUPPORTED_EXTS and not name.startswith("__MACOSX"):
                    try:
...
def _search_code(pattern: str, file_contents: dict) -> list:
    hits = []
    regex = re.compile(re.escape(pattern), re.IGNORECASE)
    for filepath, content in file_contents.items():
        for lineno, line in enumerate(content.splitlines(), 1):
            if regex.search(line):
                hits.append({"file": filepath, "line": lineno, "snippet": line.strip()[:200]})
                if len(hits) >= 50:
                    return hits
    return hits
...
<div style="background:#0d1117;border:1px solid #21262d;border-radius:8px;
  padding:10px 14px;margin-bottom:6px;font-family:monospace;font-size:0.8rem">
  <span style="color:#64748b">{m['file']}</span>
  <span style="color:#475569"> · line {m['line']}</span><br>
  <span style="color:#aed581">{m['snippet']}</span>
</div>""", unsafe_allow_html=True)

tab_cis, tab_cve = st.tabs([
...
with tab_cis:
    ...
    # ── Upload + Run ──────────────────────────────────────────────────────────
    col_upload, col_code_cis, col_help = st.columns([3, 3, 2])
    with col_upload:
    ...
    with col_code_cis:
        st.markdown('<div class="section-title">🧬 Codebase ZIP (optional)</div>', unsafe_allow_html=True)
        cis_code_zip = st.file_uploader(
            "Upload source code ZIP for exposure analysis",
            type=["zip"],
            key="cis_code_zip",
            label_visibility="collapsed",
            help="If provided, each FAIL control description is searched in your code to determine if the vulnerable pattern is present.",
        )
        if cis_code_zip:
            st.caption("✅ Codebase uploaded — exposure verdict will be added to each FAIL control after analysis.")
        else:
            st.caption("Optional: upload your source code ZIP to get Exposed / Not Exposed verdicts per control.")
    with col_help:
...
""", unsafe_allow_html=True)
    ...
    if run_btn:
        ...
        with st.spinner("Parsing CIS benchmark controls…"):
            ...
            # store codebase if uploaded
            if cis_code_zip is not None:
                st.session_state["cis_code_bytes"] = cis_code_zip.read()
            else:
                st.session_state.pop("cis_code_bytes", None)
                st.session_state.pop("cis_file_contents", None)
...
"""
    ...
    # ── Codebase Exposure (CIS) ───────────────────────────────────────────────
    if (
        st.session_state.get("cis_priority_done")
        and "cis_code_bytes" in st.session_state
        and "cis_file_contents" not in st.session_state
    ):
        with st.spinner("🧬 Scanning codebase for exposed controls…"):
            fc = _load_zip_codebase(st.session_state["cis_code_bytes"])
            st.session_state["cis_file_contents"] = fc
            df_cis = st.session_state["cis_df"]
            exposures, patterns_col, matches_col = [], [], []
            for _, row in df_cis.iterrows():
                if row["Result"] == "PASS":
                    exposures.append("—")
                    patterns_col.append("")
                    ...
                    verdict, pats, hits = _run_exposure(row["Description"], fc)
                    exposures.append(verdict)
                    patterns_col.append(pats)
                    matches_col.append(hits)
            df_cis["Exposure"] = exposures
            df_cis["_exposure_patterns"] = patterns_col
            df_cis["_exposure_matches"] = matches_col
            st.session_state["cis_df"] = df_cis
...
<div class="metric-card">
  ...
  <div class="lbl">{label}</div>
        ...
        # ── Codebase Exposure Results (CIS) ───────────────────────────────────
        if "Exposure" in df_cis.columns:
            st.markdown('<div class="section-title">🧬 Codebase Exposure Analysis</div>', unsafe_allow_html=True)
            exp_counts = df_cis["Exposure"].value_counts()
            xc1, xc2, xc3, xc4 = st.columns(4)
            for col, key, color, label in [
                (xc1, "🔴 Exposed / Used",       "#f87171", "Exposed / Used"),
                (xc2, "🟢 Not Exposed",           "#86efac", "Not Exposed"),
                (xc3, "⚠️ Cannot be Concluded",   "#fbbf24", "Cannot Conclude"),
                (xc4, "—",                         "#64748b", "PASS (skipped)"),
            ]:
                with col:
                    st.markdown(f"""
<div class="metric-card">
  <div class="val" style="color:{color}">{exp_counts.get(key, 0)}</div>
  <div class="lbl">{label}</div>
</div>""", unsafe_allow_html=True)

            st.markdown("&nbsp;", unsafe_allow_html=True)
            # Exposed FAIL controls deep-dive
            exposed_cis = df_cis[df_cis["Exposure"] == "🔴 Exposed / Used"]
            if not exposed_cis.empty:
                st.markdown("**🔴 Exposed controls — code match details:**")
                for _, erow in exposed_cis.iterrows():
                    with st.expander(f"{erow['Control ID']}  —  {erow['Description'][:80]}"):
                        st.markdown(f"**Matched patterns:** `{erow.get('_exposure_patterns','')}`")
                        _render_code_matches(erow.get("_exposure_matches", []))
        ...
        with ec1:
            buf = BytesIO()
            export_cis = df_cis.drop(columns=[c for c in ["_exposure_patterns","_exposure_matches"] if c in df_cis.columns])
            with pd.ExcelWriter(buf, engine="openpyxl") as writer:
                export_cis.sort_values("Priority").to_excel(writer, index=False, sheet_name="CIS Controls")
            st.download_button(
                "⬇  Download Excel (.xlsx)",
                data=buf.getvalue(),
        ...
        with ec2:
            csv_bytes = export_cis.sort_values("Priority").to_csv(index=False).encode()
            st.download_button(
                "⬇  Download CSV",
                data=csv_bytes,
...
with tab_cve:
    ...
    col_up, col_code_cve, col_opts = st.columns([3, 3, 2])
    with col_up:
    ...
    with col_code_cve:
        st.markdown('<div class="section-title">🧬 Codebase ZIP (optional)</div>', unsafe_allow_html=True)
        cve_code_zip = st.file_uploader(
            "Upload source code ZIP for exposure analysis",
            type=["zip"],
            key="cve_code_zip",
            label_visibility="collapsed",
            help="If provided, each CVE description is searched in your codebase. Results: Exposed / Not Exposed / Cannot be Concluded.",
        )
        if cve_code_zip:
            st.caption("✅ Codebase uploaded — exposure verdict will be added per CVE after analysis.")
        else:
            st.caption("Optional: upload your source code ZIP to see which CVEs are actually reachable in your code.")
...
""", unsafe_allow_html=True)
    ...
    if run_cve:
        ...
        # store codebase if uploaded
        if cve_code_zip is not None:
            st.session_state["cve_code_bytes"] = cve_code_zip.read()
            st.session_state.pop("cve_exposure_done", None)
        else:
            st.session_state.pop("cve_code_bytes", None)
            st.session_state.pop("cve_exposure_done", None)
    ...
    if "cve_enriched" in st.session_state:
        ...
        # ── Run codebase exposure if ZIP was provided ──────────────────────────
        if "cve_code_bytes" in st.session_state and not st.session_state.get("cve_exposure_done"):
            with st.spinner("🧬 Scanning codebase for CVE exposure…"):
                cve_fc = _load_zip_codebase(st.session_state["cve_code_bytes"])
                st.session_state["cve_file_contents"] = cve_fc
                for cid, inf in enriched.items():
                    desc = inf.get("nvd", {}).get("description") or inf.get("synopsis", "")
                    verdict, pats, hits = _run_exposure(desc, cve_fc)
                    inf["exposure_verdict"]  = verdict
                    inf["exposure_patterns"] = pats
                    inf["exposure_matches"]  = hits
                st.session_state["cve_enriched"] = enriched
                st.session_state["cve_exposure_done"] = True

        # Flatten to DataFrame
        rows_df = []
        has_exposure = any("exposure_verdict" in inf for inf in filtered.values())
        for cid, inf in sorted(filtered.items(), key=lambda x: SEVERITY_ORD.get(x[1].get("nessus_severity", "None"), 99)):
            ...
            if has_exposure:
                row["Exposure Verdict"]   = inf.get("exposure_verdict", "—")
                row["Matched Patterns"]   = inf.get("exposure_patterns", "—")
            rows_df.append(row)
...
<div class="metric-card">
  ...
  <div class="lbl">{label}</div>
        ...
        # ── Codebase Exposure Results (CVE) ───────────────────────────────────
        if has_exposure:
            st.markdown('<div class="section-title">🧬 Codebase Exposure Analysis</div>', unsafe_allow_html=True)
            all_verdicts = [inf.get("exposure_verdict","") for inf in filtered.values()]
            n_exposed  = sum(1 for v in all_verdicts if v.startswith("🔴"))
            n_safe     = sum(1 for v in all_verdicts if v.startswith("🟢"))
            n_unknown  = sum(1 for v in all_verdicts if v.startswith("⚠️"))

            xm1, xm2, xm3 = st.columns(3)
            for col, val, color, label in [
                (xm1, n_exposed,  "#f87171", "🔴 Exposed / Used"),
                (xm2, n_safe,     "#86efac", "🟢 Not Exposed"),
                (xm3, n_unknown,  "#fbbf24", "⚠️ Cannot be Concluded"),
            ]:
                with col:
                    st.markdown(f"""
<div class="metric-card">
  ...
  <div class="lbl">{label}</div>
            ...
            # Exposed CVEs deep-dive
            exposed_cves = {cid: inf for cid, inf in filtered.items()
                            if inf.get("exposure_verdict","").startswith("🔴")}
            if exposed_cves:
                st.markdown("**🔴 Exposed CVEs — code match details:**")
                sel_ecve = st.selectbox(
                    "Select an exposed CVE to inspect",
                    options=list(exposed_cves.keys()),
                    key="cve_exp_select",
                    format_func=lambda c: f"{c}  —  {filtered[c].get('nessus_severity','?')}  |  {filtered[c].get('exposure_patterns','')[:60]}",
                )
                if sel_ecve:
                    einf = exposed_cves[sel_ecve]
                    edesc = einf.get("nvd", {}).get("description") or einf.get("synopsis", "")
                    st.markdown(f"""
<div style="background:#1e293b;border:1px solid #334155;border-radius:10px;
  padding:16px 20px;margin-bottom:12px">
  <div style="color:#38bdf8;font-size:0.78rem;font-weight:600;
    text-transform:uppercase;letter-spacing:.8px;margin-bottom:6px">Description</div>
  <div style="color:#cbd5e1;font-size:0.87rem;line-height:1.6">{edesc[:500]}</div>
  <div style="margin-top:10px;color:#fbbf24;font-size:0.8rem">
    <strong>Matched patterns:</strong> {einf.get('exposure_patterns','')}
  </div>
</div>""", unsafe_allow_html=True)
                    _render_code_matches(einf.get("exposure_matches", []))
